...
export default function AppError({ error, reset }: { error: Error; reset: () => void }) {
  ...
  useEffect(() => {
    setMessage(error?.message || 'Impossible de charger cette section pour le moment.')
    console.error('Erreur capturée (app):', error)
  }, [error])
  ...
  return (
    <div className="min-h-screen bg-canvas flex items-center justify-center p-6">
      <div className="max-w-lg w-full bg-white border border-border rounded-2xl shadow-soft p-8 space-y-4 text-center">
        ...
        <h1 className="text-2xl font-bold text-ink">Section indisponible</h1>
